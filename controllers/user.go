...
func RefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	existingToken := r.Header.Get("Authorization")
	if existingToken == "" {
		utils.RespondWithError(w, http.StatusBadRequest, "No token provided")
		return
	}

	token, err := jwt.Parse(existingToken, func(t *jwt.Token) (interface{}, error) {
	...
	newToken, tokenErr := utils.GenerateToken(int(claims["user_id"].(float64)))
	if tokenErr != nil {
		utils.RespondWithError(w, http.StatusInternalServerError, "Internal Server error")
		return
	}
